        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                var parameters = new Dictionary<object, object>();
                 ...
                 var projectsList = projectsApi.GetProjects(parameters);
                 var projects = projectsList;
                 var projectId = projects[1].project_id;
                 ...
                 var updatedprjct = projectsApi.Update(projectId, updateInfo);
                 Console.WriteLine("{0},{1},{2}", updatedprjct.project_name, updatedprjct.billing_type, updatedprjct.status);
                 var delprjct = projectsApi.Delete(projects[2].project_id);
                 ...
                 var updatedTask = projectsApi.UpdateTask(projectId, taskId, updateInfo1);
                 Console.WriteLine("{0},{1}", updatedTask.task_name, updatedTask.task_id);
                 var delTask = projectsApi.DeleteTask(projectId, tasks[1].task_id);
                ...
                Console.WriteLine("{0},{1},{2}", user2.user_name, user2.user_role, user2.rate);
                 ...
                 var UpdatedUser = projectsApi.UpdateUser(projectId, userId, updateInfo2);
                 Console.WriteLine("{0},{1},{2}", UpdatedUser.user_name, UpdatedUser.user_role, UpdatedUser.rate);
                 var deleteUser = projectsApi.DeleteUser(projectId, users[2].user_id);
                 ...
                 var updatedTimeEntry = projectsApi.UpdateTimeEntry(timeEnteryId, updateInfo3);
                 Console.WriteLine("time entry of id {0} for the project {1} of user {2} of log time:{3}\n", updatedTimeEntry.time_entry_id, updatedTimeEntry.project_name, updatedTimeEntry.user_name, updatedTimeEntry.log_time);
                 var deleteMsg = projectsApi.DeleteTimeEntry(timeEntries[1].time_entry_id);
                 Console.WriteLine(deleteMsg);
                 var parameters3 = new Dictionary<object, object>();
                 parameters3.Add("time_entry_ids", timeEnteryId + "," + timeEntries[1].time_entry_id);
                ...
                var commentId = comments[2].comment_id;
                ...
                var deleteComment = projectsApi.DeleteComment(projectId, commentId);
                Console.WriteLine(deleteComment);
                var parameters4 = new Dictionary<object, object>();
                var invoicesList = projectsApi.GetInvoices(projectId, parameters4);
